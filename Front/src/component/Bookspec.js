...
const Bookspec = () => {
    ...
    const add = async () => {
        if (logstate == 'false')
            ...
            let ans = await fetch('http://localhost:1337/add-cart?id=' + id + '&token=' + cookies.get('token'));
            ans = await ans.json();
            console.log(ans)
        }
    }
        ...
        <>
            ...
            <div style={{ padding: '2%' }} className='bg-dark'>
                ...
                <div className="row" style={{ marginBottom: '2%' }}>
                    ...
                    <div className="col-sm-4" style={{ color: 'white', direction: 'rtl', textAlign: 'right' }} >
                        <button onClick={add} style={{ width: '100%' }} className="form-control btn btn-primary submit px-3">
