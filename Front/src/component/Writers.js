...
import Noimg from '../unkperson.jpg'
import {
  BrowserRouter,
  Routes,
  Route,
...
const Writers = () => {
  ...
  useEffect(async () => {
    try {
      let ans = await fetch(process.env.REACT_APP_BACKEND+'/author/list');
      ans = await ans.json();
      console.log(ans);
      let t = [];
      ans.forEach((item, index) => {
        if (item['image'] != undefined)
          ...
          t.push(
            <div className="col-sm-3" style={{ display: 'flex' }}>
              <div className="card">
                <img className="card-img-top" src={Noimg} height='400px' alt="Card image cap" />
                <div className="card-body bg-dark" style={{ textAlign: 'center' }}>
                  <Link to={"/writers/" + item['name']}>
                    {item['name']}
                    <svg xmlns="http://www.w3.org/2000/svg" style={{ marginRight: '2%' }} width="16" height="16" fill="currentColor" class="bi bi-search" viewBox="0 0 16 16">
  ...
  return (
    <>
      <h3 style={{ textAlign: 'center', marginTop: '30px' }}>نویسندگان</h3>
      <div style={{ padding: '2%' }}>
